...
/**
 * Base Singleton Fields with Content Sections
 *
 * Alternative to baseSingletonFields.ts - uses contentSection objects
 * instead of separate contentBlocks and faqBlocks arrays.
 *
 * This structure:
 * - Groups content + FAQs together semantically
 * - Creates clear entity boundaries for AI retrieval
 * - Allows flexible drag/drop ordering of sections
 * - Aligns with AI Search Playbook 1.3.1
 *
 * Use for: Travel to Islay, Explore Islay pages
 * Use baseSingletonFields for: Homepage, About, Contact (simpler structure)
 */
export const baseSingletonFieldsWithSections = [
  defineField({
    name: 'title',
  ...
  defineField({
    ...
    description: 'Optional introduction text before sections (appears after hero, before first section)',
  }),

  // Page Sections (Content + FAQs grouped)
  defineField({
    name: 'pageSections',
    title: 'Page Sections',
    type: 'array',
    ...
    description:
      'Sections containing content blocks and related FAQ blocks. Each section = one semantic unit (e.g., "Ferry to Islay"). Drag to reorder.',
  }),
